...
const Navbar = () => {
    ...
    return (
        <nav className='flex md:flex-row flex-col items-center md:justify-between justify-center bg-black text-white p-1 px-4'>
            ...
            <div className='relative flex md:flex-row flex-col gap-2'>
                {
                    session && <> <button id="dropdownDefaultButton" onClick={() => setshowDropdown(!showDropdown)} data-dropdown-toggle="dropdown" onBlur={() => {
                        setTimeout(() => {
                            setshowDropdown(false)
                        }, 300);
                    }}
                        className=" text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm mx-4 px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">{session.user.email}<svg className="w-2.5 h-2.5 ms-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                            <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m1 1 4 4 4-4" />
                        </svg>
                    </button>


                        <div id="dropdown" className={`z-10 ${showDropdown ? "" : "hidden"} absolute left-[90px] top-[50px] bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700`}>
